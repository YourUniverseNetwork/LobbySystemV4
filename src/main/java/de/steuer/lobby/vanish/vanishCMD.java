    ...
    public boolean onCommand(final CommandSender sender, final Command command, final String label, final String[] args) {
        ...
        if (args.length == 1) {
            ...
            if (vanishManager.isVanished(player)) {
                ...
                sender.sendMessage(prefix + ChatColor.GREEN + "Der Spieler ist jetzt nicht mehr im Vanish");
            }
            else {
            ...
            if (sender instanceof Player) {
                ...
                if (vanishManager.isVanished(player)) {
                    vanishManager.setVanished(player, false);
                    sender.sendMessage(prefix + ChatColor.GREEN + "Du bist jetzt nicht mehr im Vanish");
                }
                else {
                    vanishManager.setVanished(player, true);
